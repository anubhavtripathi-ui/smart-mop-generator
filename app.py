...
def build_mop(
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        if sec_key == "sop":
            ...
            # Skip an image here only up to the number of images the
            # verbatim Activity MOP clone (step 1) already placed at their
            # original, correct position — re-adding those duplicated the
            # image and could collide drawing IDs ("Word found unreadable
            # content"). We count actual images, not just "any text",
            # because a clone with text but zero images must NOT suppress
            # every leftover image — that was making no images appear at
            # all when the real images ended up bucketed outside "sop".
            # Attachments (Excel/OLE) are NOT duplicated by the verbatim
            # clone (their relationship can't survive a raw XML copy into a
            # different package), so they still go through unconditionally.
            _BLIP_TAG = "{http://schemas.openxmlformats.org/drawingml/2006/main}blip"
            _sop_img_count = sum(
                len(elem.findall(".//" + _BLIP_TAG))
                for elem in (mop_sections or {}).get("sop", [])
            )
            _skipped_img_count = 0
            while media_idx < len(media_queue):
                m = media_queue[media_idx]; media_idx += 1
                if m.kind == "image":
                    if _skipped_img_count < _sop_img_count:
                        _skipped_img_count += 1
