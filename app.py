        ...
        if not ole_rids_here:
            ...
            for blip in p_xml.findall(f".//{{{_NS_BLIP}}}blip"):
                ...
                if ext.lower() in ("emf", "wmf"):
                    # Try PIL conversion; if it fails, skip
                    if not _PIL_AVAILABLE:
                        continue
                    try:
                        _PIL_Image.open(io.BytesIO(blob)).load()
                        # PIL can open this EMF/WMF — allow it through
                    except Exception:
                        continue  # truly unreadable metafile — skip
                if len(blob) < 300:   # skip only truly empty/placeholder blobs
                    continue
...
def _normalize_image_bytes(img_bytes: bytes) -> bytes:
    """
    Convert any image format (TIFF, BMP, WMF, corrupt JPEG, etc.) to PNG
    using Pillow so that python-docx can always embed it correctly.
    Returns original bytes if PIL not available or conversion fails.
    """
    if not _PIL_AVAILABLE:
        return img_bytes
    try:
        buf_in  = io.BytesIO(img_bytes)
        img     = _PIL_Image.open(buf_in)
        img.load()                          # force decode — catches corrupt files early
        # Convert palette/RGBA/CMYK → RGB for JPEG compatibility; keep PNG for lossless
        ...
        return img_bytes                    # fall back to original bytes


def _make_image_xml(doc: Document, img_bytes: bytes, width_inches=5.5):
    """
    Embed img_bytes as an inline image paragraph.
    Always normalises to PNG first so WMF/TIFF/BMP/corrupt-JPEG all work.
    """
    safe_bytes = _normalize_image_bytes(img_bytes)
    tmp_p = doc.add_paragraph()
    run   = tmp_p.add_run()
    run.add_picture(io.BytesIO(safe_bytes), width=Inches(width_inches))
    p_xml = tmp_p._element
    p_xml.getparent().remove(p_xml)
    return p_xml
