...
def extract_sections(doc: Document) -> dict:
    ...
    for child in body_elem:
        ...
        # ── Heading 1 is ALWAYS a section boundary ────────────────────────────
        # Even when it doesn't map to a known key (e.g. unmapped sub-headings),
        # we must SKIP it as body content.  Failing to do so causes Heading2
        # content (Phase 1/2/3 under Node Connectivity) to leak into the wrong
        # section bucket because the loop cursor never advanced to a new key.
        if is_h1:
            if key_from_text:
                current_key = key_from_text
            # Always skip — Heading1 is never body content
            continue

        # ── Heading 2+ : NEVER switch sections — always body content ───────────
        # Heading2/3 paragraphs (e.g. "Phase 2: Inventory and Vendor Identification")
        # must NOT trigger a section switch even if their text contains a keyword
        # like "inventory".  Only Heading1 defines section boundaries.
        # They fall through and are added as body content under current_key.
        if is_heading_style:
            pass  # fall through to body-content handling

        # ── Plain paragraph with strong section-keyword match ─────────────────
        # Only short plain-text paragraphs that EXACTLY match a section keyword
        # (not Heading-styled) may switch the section.
        elif key_from_text and len(text) < 120:
            current_key = key_from_text
            continue
...
def _clone_para(src_elem, num_map: dict = None, full_para_text: str = None):
    ...
    # ── Alignment fix: non-list body paragraphs must start at the same left
    # margin as headings (no extra indent from the source document).
    # List paragraphs keep their indent (bullet/numbered hanging indent).
    pPr_cl = cloned.find(f"{{{_W}}}pPr")
    if pPr_cl is not None and not is_heading:
        numPr_cl = pPr_cl.find(f"{{{_W}}}numPr")
        ind_cl   = pPr_cl.find(f"{{{_W}}}ind")
        if numPr_cl is None and ind_cl is not None:
            # Plain body paragraph with an unwanted left indent — reset to 0
            left_val = ind_cl.get(qn("w:left"), "0")
            if int(left_val) > 0:
                # Keep hanging if set (rare), but zero the left
                ind_cl.set(qn("w:left"), "0")
                if ind_cl.get(qn("w:hanging")):
                    ind_cl.attrib.pop(qn("w:hanging"), None)
...
def build_mop(
    ...
    # Activity name subtitle under title + underline below "METHOD OF PROCEDURE"
    for child in list(body):
        if child.tag.split("}")[-1] != "p":
            continue
        se = child.find(".//" + qn("w:pStyle"))
        if se is not None and se.get(qn("w:val")) == "Title":
            # ── Add bottom border (underline) to the Title paragraph itself ──
            title_pPr = child.find(".//" + qn("w:pPr"))
            if title_pPr is None:
                title_pPr = OxmlElement("w:pPr"); child.insert(0, title_pPr)
            title_pBdr = OxmlElement("w:pBdr")
            title_bot  = OxmlElement("w:bottom")
            title_bot.set(qn("w:val"),   "single")
            title_bot.set(qn("w:sz"),    "12")      # 1.5pt line
            title_bot.set(qn("w:space"), "4")
            title_bot.set(qn("w:color"), "000000")  # black
            title_pBdr.append(title_bot)
            title_pPr.insert(0, title_pBdr)

            # ── Activity name subtitle paragraph ──────────────────────────────
    ...
    # This preserves exact list formats (decimal/lowerLetter/bullet) from sources.
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        if sec_key == "sop":
            ...
            # ── UAT: numbered items, each with Condition/Measurement/Pass/Fail sub-lines ──
            #
            # Problem: Solution doc may write all content in ONE paragraph like:
            #   "Pre-check execution validation• Condition: X• Measurement Method: Y•
            #    Pass: Z• Fail: W• XML plan import validation• Condition: ..."
            #
            # Strategy:
            #   1. Collect all raw text from paragraphs
            #   2. Split every paragraph on the bullet char '•' to get atomic fragments
            #   3. Classify each fragment: TITLE vs SUB (Condition/Measurement/Pass/Fail)
            #   4. Group: each TITLE starts a new UAT item; SUB lines go under it
            #   5. Render: numbered bold-black title + indented '• sub-line' entries
            import re as _re2

            _num_re   = _re2.compile(r'^\d+[\.\)]\s*')
            # Sub-line keywords — these are never titles
            _sub_kw   = _re2.compile(
                r'^(Condition|Measurement\s*Method|Pass|Fail)\s*:',
                _re2.IGNORECASE
            )
            ...
            def _uat_heading_xml(num, title):
                """Numbered UAT item — bold, BLACK, 11pt Calibri."""
                p   = OxmlElement("w:p")
                pPr = OxmlElement("w:pPr")
                sp  = OxmlElement("w:spacing")
                sp.set(qn("w:before"), "160"); sp.set(qn("w:after"), "40")
            ...
            def _uat_subline_xml(line):
                """Indented bullet sub-line — Condition / Measurement Method / Pass / Fail."""
                p   = OxmlElement("w:p")
                pPr = OxmlElement("w:pPr")
                ind = OxmlElement("w:ind"); ind.set(qn("w:left"), "440")
                sp  = OxmlElement("w:spacing")
                sp.set(qn("w:before"), "0"); sp.set(qn("w:after"), "30")
                ...
                sz  = OxmlElement("w:sz"); sz.set(qn("w:val"), "20")
                col = OxmlElement("w:color"); col.set(qn("w:val"), "000000")
                rpr.append(fn); rpr.append(sz); rpr.append(col); r.append(rpr)
                t   = OxmlElement("w:t")
                t.set("{http://www.w3.org/XML/1998/namespace}space", "preserve")
                t.text = line; r.append(t); p.append(r)
                return p

            # ── Step 1: collect ALL raw paragraph texts ──────────────────────
            raw_para_texts = []
            for p_elem in content_elems:
                txt = _get_txt(p_elem)
                if txt:
                    raw_para_texts.append(txt)

            # ── Step 2: split every paragraph on '•' to get atomic fragments ─
            # A paragraph may be:
            #   Case A: separate paragraphs per item (Activity MOP style)
            #   Case B: everything jammed in one paragraph with '•' separators
            # Splitting on '•' handles BOTH cases uniformly.
            fragments = []
            for para_text in raw_para_texts:
                # Also handle numbered prefix like "1. Pre-check..."
                parts = [p.strip() for p in para_text.split('•')]
                for part in parts:
                    if part:
                        fragments.append(part)

            # ── Step 3 & 4: classify fragments and group into UAT items ──────
            # TITLE  = fragment that does NOT start with a sub-keyword
            # SUB    = fragment that starts with Condition/Measurement/Pass/Fail
            groups = []   # list of {title, subs}
            cur_title = None
            cur_subs  = []

            for frag in fragments:
                # Strip leading number "1. " or "1) "
                clean = _num_re.sub('', frag).strip()
                if not clean:
                    continue

                if _sub_kw.match(clean):
                    # It's a sub-line — belongs to current item
                    if cur_title is None:
                        # Sub-line before any title seen — skip (shouldn't happen)
                        continue
                    cur_subs.append(clean)
                else:
                    # It's a title — start a new UAT item
                    if cur_title is not None:
                        groups.append((cur_title, list(cur_subs)))
                    cur_title = clean
                    cur_subs  = []
            ...
            # ── Step 5: render ───────────────────────────────────────────────
            for item_num, (title, subs) in enumerate(groups, start=1):
                # Numbered bold-black title
                hdg = _uat_heading_xml(item_num, title[:80].rstrip(',;- '))
                _insert_after(cursor, hdg); cursor = hdg

                # Each sub-line indented with • prefix
                for sl in subs:
                    sl = sl.strip()
                    if not sl: continue
                    line_with_bullet = f"• {sl}" if not sl.startswith('•') else sl
                    sxml = _uat_subline_xml(line_with_bullet)
                    _insert_after(cursor, sxml); cursor = sxml

                # Blank spacer between UAT items
