...
def check_template(path):
    if not os.path.exists(path):
        ...
        Upload it to GitHub → <code>templates/</code> folder.
...
if not os.path.exists(selected_template_path):
    st.markdown(
        f'<div style="color:#fca5a5;font-size:0.85rem;margin-top:4px;">'
        f'⚠️ {selected_label} not available. Upload '
        f'<code>{TEMPLATE_OPTIONS[selected_label]}</code> to GitHub → templates/ folder.</div>',
        unsafe_allow_html=True
    )
    ...
    if errors:
        ...
        try:
            ...
            if result['success']:
                ...
                sop_ref_path = result.get('sop_ref_path')
                ...
                # Build ZIP with MOP + SOP Reference
                ...
                with _zf.ZipFile(zip_buf, 'w', _zf.ZIP_DEFLATED) as zf:
                    ...
                    if sop_ref_path and os.path.exists(sop_ref_path):
                        sop_ref_name = os.path.basename(sop_ref_path).split('__', 1)[-1]
                        zf.write(sop_ref_path, sop_ref_name)
                ...
                <div class="success-box">
                    ...
                    <p>📦 Download ZIP contains:</p>
                    <p>&nbsp;&nbsp;📄 <strong>{output_filename}</strong> — Output MOP</p>
                    <p>&nbsp;&nbsp;📎 <strong>{sop_ref_name if sop_ref_name else 'SOP Reference'}</strong> — Complete input copy</p>
                    <p>🔒 No data stored or retained on server</p>
                    <br>
                    <p><strong style="color:#93c5fd;">Sections from your input ({len(filled)}):</strong></p>
                    <div class="chip-row">{filled_chips if filled_chips else '<span style="color:#64748b;font-size:0.8rem;">None detected</span>'}</div>
                    <br>
                    <p><strong style="color:#fde68a;">Auto-filled sections ({len(auto)}):</strong></p>
                    <div class="chip-row">{auto_chips if auto_chips else '<span style="color:#64748b;font-size:0.8rem;">None</span>'}</div>
                </div>
                """, unsafe_allow_html=True)
