    ...
    for h_elem, sec_key in ordered_sections:
        ...
        if sec_key == "sop":
            ...
            def _uat_heading_xml(num, title):
                """Numbered UAT item title — bold, BLACK (not blue), 11pt."""
                p = OxmlElement("w:p")
                pPr = OxmlElement("w:pPr")
                ...
                b  = OxmlElement("w:b")
                col= OxmlElement("w:color"); col.set(qn("w:val"),"000000")  # BLACK — not blue
                rpr.append(fn); rpr.append(sz); rpr.append(b); rpr.append(col)
                r.append(rpr)
                t = OxmlElement("w:t")
                t.set("{http://www.w3.org/XML/1998/namespace}space","preserve")
            ...
            def _uat_subline_xml(line):
                """Indented bullet sub-line for UAT item (Condition/Measurement/Pass/Fail)."""
                p = OxmlElement("w:p")
                pPr = OxmlElement("w:pPr")
            ...
            # ── UAT format: each paragraph = one UAT item ────────────────────
            # Expected paragraph format (from Activity MOP / Solution Doc):
            #   Title line (bold or numbered)
            #   • Condition: ...
            #   • Measurement Method: ...
            #   • Pass: ...
            #   • Fail: ...
            # We render: numbered title in bold black + each bullet sub-line indented.

            # Collect all text lines from content elements
            raw_lines = []
            for p_elem in content_elems:
                txt = _get_txt(p_elem)
                if txt:
            ...
            # Group lines: a new group starts when we see a numbered line,
            # or a bold-looking title (no bullet prefix), or when we see
            # '• Condition:' after already having gathered a group's sub-lines.
            _bullet_re  = _re2.compile(r'^[•\-\*]\s*(Condition|Measurement|Pass|Fail)', _re2.IGNORECASE)
            _cond_re    = _re2.compile(r'^[•\-\*]?\s*Condition\s*:', _re2.IGNORECASE)

            groups  = []
            cur     = []
            for line in raw_lines:
                is_num    = bool(_num_re.match(line))
                is_cond   = bool(_cond_re.match(line))
                # A '• Condition:' after we already have sub-lines in cur → new group
                if is_cond and len(cur) > 1:
                    groups.append(cur); cur = []
                if is_num:
                    if cur: groups.append(cur)
                    # Strip leading number from title
            ...
            for grp in groups:
                ...
                item_num += 1
                first = grp[0]
                # Title: first line (strip any leading bullet/number)
                title = _re2.sub(r'^[•\-\*]\s*', '', first).strip()
                title = title[:80].rstrip(',;- ')
                sub_lines = grp[1:]

                # Render numbered title (bold, black)
                hdg = _uat_heading_xml(item_num, title)
                _insert_after(cursor, hdg); cursor = hdg

                # Render each sub-line (Condition / Measurement / Pass / Fail)
                for sl in sub_lines:
                    sl = sl.strip()
                    if not sl: continue
