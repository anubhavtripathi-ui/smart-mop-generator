...
# ── Activity MOP heading → section key mapping ──────────────────
# Maps common Activity MOP heading keywords to our 12 section keys.
# Longer / more-specific phrases are checked first (order matters).
MOP_HEADING_MAP = [
    # objective / intro
    ("objective",                    "objective"),
    ...
    ("test criteria",                "acceptance_criteria"),
    # assumptions
...
def normalize_mop_heading(text: str):
    """Map an Activity MOP heading to one of our 12 section keys, or return None."""
...
# ─────────────────────────────────────────────────────────────────
# ACTIVITY MOP — SECTION CONTENT EXTRACTOR
# ─────────────────────────────────────────────────────────────────
def extract_mop_sections(mop_bytes: bytes) -> dict:
    """
    Parse the Activity MOP and distribute its paragraphs into our 12 section
    buckets using MOP_HEADING_MAP keyword matching.

    Rules:
    - A paragraph is a "heading" if:
        (a) its Word style starts with "Heading", OR
        (b) it is short (<= 80 chars), bold/ALL-CAPS, and matches a keyword.
    - Content under a matched heading goes into that section bucket.
    - Content under an UNMATCHED heading (or before any heading) goes into
      the "sop" bucket (Section 10) — the catch-all for raw MOP detail.
    - Empty paragraphs between content are preserved so layout is maintained.
    - Document-level boilerplate (TOC lines, revision tables, header/footer
      text) is filtered out.
    - Tables are included: each table row is converted to a plain-text
      paragraph element so it flows naturally into the section.
    """
    doc = Document(io.BytesIO(mop_bytes))
    mop_secs  = {k: [] for k in SECTION_KEYS}
    current   = "sop"          # default bucket for unmatched content

    # Helper: detect if a paragraph is a heading in the MOP
    def _is_mop_heading(para):
        ...
        if not txt or len(txt) > 100:
            return False
        # Bold-only short paragraph  OR  ALL-CAPS short paragraph
    ...
    _SKIP_RE = re.compile(
        ...
        r'|header|footer|page \d|confidential|ericsson|method of procedure'
        r'|prepared by|approved by|document number|version no)',
        re.IGNORECASE
    )

    # Walk body elements in order (paragraphs + tables)
    ...
    for child in body_elem:
        ...
        if tag == "p":
            para_obj = None
            # Find the matching python-docx Paragraph for this element
            ...
            if _is_mop_heading(para_obj):
                ...
                if mapped:
                    ...
                    current = "sop"   # unrecognised heading → catch-all
                continue              # heading itself is not copied

            # ── Boilerplate filter ────────────────────────────────
            if not text:
                # preserve empty paragraphs inside a section for spacing
                if current in mop_secs:
                    mop_secs[current].append(deepcopy(child))
                continue

            if _SKIP_RE.match(text):
                continue
            if re.match(r'^\d+[\.\)]\s+\w.{0,60}Page\s+\d+', text):
                continue   # TOC line

            mop_secs[current].append(deepcopy(child))

        elif tag == "tbl":
            # Convert every non-empty table row to a simple paragraph element
            # so it integrates naturally with the surrounding text flow.
            for row in child.findall(f".//{{{_W}}}tr"):
                ...
                # Build a minimal <w:p> with this text preserving Calibri font
                ...
                r_el.append(t_el)
                p_el.append(r_el)
                mop_secs[current].append(p_el)

    # Strip trailing empty paragraphs from each bucket
    for key in mop_secs:
        while mop_secs[key]:
            last_txt = "".join(
                t.text or ""
                for t in mop_secs[key][-1].findall(".//" + qn("w:t"))
            ).strip()
            if not last_txt:
                mop_secs[key].pop()
            else:
...
def extract_sections(doc: Document) -> dict:
    ...
    tables, empty paragraphs, and sub-headings exactly as they appear in the
    Solution Document.  Tables are included verbatim (not flattened) so the
    output DOCX renders them correctly.
    """
    ...
    for child in body_elem:
        ...
        if tag == "tbl":
            # Tables belong to whatever section is currently active
        ...
        # ── It's a paragraph ─────────────────────────────────────
        # Find the python-docx Paragraph wrapper for style access
        para_obj = None
        for p in doc.paragraphs:
            if p._p is child:
                para_obj = p
        ...
        # Include paragraph — even empty ones (they carry spacing/formatting)
...
def _embed_attachment_into_docx(doc_bytes: bytes, media_item, attach_idx: int):
    try:
        ...
        # Use oleObject relationship type — "package" causes Word corruption errors
...
def build_mop(
    ...
    today_str:       str,
    media_items:     list,
    mop_sections:    dict = None,   # keyed by section_key → list of XML elements
) -> tuple:
    ...
    for h_elem, sec_key in ordered_sections:
        # Clear template boilerplate under heading (paragraphs AND tables)
        ...
        for child in list(body):
            ...
            if ctag == "p":
                ...
                # Remove template placeholder tables (they will be replaced by content)
                to_remove.append(child)
        for e in to_remove:
            body.remove(e)
        ...
        mop_content   = (mop_sections or {}).get(sec_key, []) if sec_key != "sop" else []
        if not content_elems and not mop_content and not (mop_sections and sec_key == "sop"):
        ...
        if sec_key == "sop":
            # ── Helper: insert one XML element (p or tbl) after cursor ────────
            def _insert_elem(elem, cur):
                tag = elem.tag.split("}")[-1]
                if tag == "tbl":
                    # Tables must be inserted directly — not via _insert_after
                    # which expects paragraph elements.
                    parent = cur.getparent()
                    idx    = list(parent).index(cur)
                    parent.insert(idx + 1, deepcopy(elem))
                    return list(parent)[idx + 1]
                else:
                    cloned = _clone_para(elem)
                    _insert_after(cur, cloned)
                    return cloned

            sol_texts_seen = set()

            # ── STEP 1: MOP content first (if Activity MOP was uploaded) ──────
            if mop_sections:
                mop_sop_elems = mop_sections.get("sop", [])
                for elem in mop_sop_elems:
                    txt = "".join(t.text or "" for t in elem.findall(".//" + qn("w:t"))).strip()
                    if txt:
                        sol_texts_seen.add(txt.lower())
                    cursor = _insert_elem(elem, cursor)

            # ── STEP 2: Solution doc SOP content ──────────────────────────────
            for p_elem in content_elems:
                txt = "".join(t.text or "" for t in p_elem.findall(".//" + qn("w:t"))).strip()
                if txt and txt.lower() in sol_texts_seen:
                    continue   # skip exact duplicates already from MOP
                if txt:
                    sol_texts_seen.add(txt.lower())
                cursor = _insert_elem(p_elem, cursor)

            # ── STEP 3: Media (images/attachments) at placeholder positions ───
            for p_elem in content_elems:
            ...
            # Helper for inserting p or tbl elements
            def _insert_elem_generic(elem, cur):
                tag = elem.tag.split("}")[-1]
                if tag == "tbl":
                    parent = cur.getparent()
                    idx    = list(parent).index(cur)
            ...
            # 1. MOP content for this section (if any)
            if mop_sections:
                for elem in mop_sections.get(sec_key, []):
                    txt = "".join(
                        t.text or "" for t in elem.findall(".//" + qn("w:t"))
                    ).strip()
                    if txt:
                        seen_texts.add(txt.lower())
                    cursor = _insert_elem_generic(elem, cursor)

            # 2. Solution doc content (deduplicated against MOP content)
            for p_elem in content_elems:
                txt = "".join(
                    t.text or "" for t in p_elem.findall(".//" + qn("w:t"))
                ).strip()
                if txt and txt.lower() in seen_texts:
                    continue
                if txt:
                    seen_texts.add(txt.lower())
                cursor = _insert_elem_generic(p_elem, cursor)
