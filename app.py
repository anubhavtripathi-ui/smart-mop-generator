...
def extract_activity_name(doc: Document) -> str:
    """Activity name = paragraph just before first 'Objective' heading."""
    ...
    if obj_idx is not None and obj_idx > 0:
        for i in range(obj_idx - 1, -1, -1):
            ...
            if re.match(r'^\d+[\.\)]\s+\w.*Page\s+\d+', text):
                continue
            # Skip metadata lines like "Customer: ..."
...
def _update_revision_table(doc: Document, activity_name: str, today_str: str):
    """Update the Revision History table with today's date and activity name."""
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                txt = cell.text.strip()
                # Update placeholder date
                if txt in ("17-04-2026", "DD-MM-YYYY", "{{date}}", ""):
                    for para in cell.paragraphs:
                        for run in para.runs:
                            if run.text.strip() in ("17-04-2026", "DD-MM-YYYY", "{{date}}"):
                                run.text = today_str
                # Update "Automation SME" or blank "Edited By" with placeholder
                # (leave as-is, we don't overwrite real names)

def _clear_section_content(doc: Document):
    """
    Clear all content paragraphs under each Heading 1 in the template,
    keeping the headings themselves intact.
    Preserves: Title, Revision History section & table, header, footer.
    """
    body = doc.element.body
    paragraphs = doc.paragraphs
    in_clearable_section = False
    paras_to_remove = []

    # Identify the Revision History heading to skip it
    for i, para in enumerate(paragraphs):
        style = para.style.name
        text  = para.text.strip()
        ...
        if style.startswith("Heading 1"):
            ...
            if key:
                in_clearable_section = True
                continue
            else:
                in_clearable_section = False
                continue

        if in_clearable_section:
            # Don't remove empty paragraphs that might be structural
            if text:
...
def _clear_and_prep_body(doc: Document):
    """Clear body except sectPr, preserving header/footer references."""
    body = doc.element.body
    header_refs = []
    footer_refs = []
    for child in body:
...
def build_mop(template_bytes: bytes, activity_name: str,
    ...
    # Update header date placeholder
    ...
    try:
        ...
        pass
    _r(p, "Revision History (To be updated by Line/SME/AA)", bold=True, size=13)

    # Revision table
    tbl = doc.add_table(rows=2, cols=4)
    tbl.style = "Table Grid"
    tbl.rows[0].cells[0].text = "Version No."
    tbl.rows[0].cells[1].text = "Revision Date"
    tbl.rows[0].cells[2].text = "Edited By"
    tbl.rows[0].cells[3].text = "Description of Change"
    tbl.rows[1].cells[0].text = "v1.0"
    tbl.rows[1].cells[1].text = today_str
    tbl.rows[1].cells[2].text = "Automation SME"
    tbl.rows[1].cells[3].text = f"Initial draft — {activity_name}"
    doc.add_paragraph()
    ...
    if images:
        _h1(doc, SECTION_LABELS["connectivity_diagram"])
...
with st.sidebar:
    ...
    1️⃣ &nbsp;Place template in <code>templates/</code><br>
...
with col_left:
    ...
    new_tmpl = st.file_uploader("Upload new / updated template (.docx)", type=["docx"],
                                key="tmpl_up", label_visibility="visible")
    if new_tmpl:
        save_dir = Path("templates")
        save_dir.mkdir(exist_ok=True)
        with open(save_dir / new_tmpl.name, "wb") as f:
