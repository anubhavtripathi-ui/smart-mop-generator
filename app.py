...
def _set_cell_text(cell, new_text: str):
    ...
    for para in cell.paragraphs:
        ...
        if para.runs:
            ...
            para.add_run(new_text).font.name = "Calibri"


def _update_cover_page(doc: Document, activity_name: str, today_str: str):
    """
    Scans ALL tables in the document (not just the revision table) to:
    1. Replace any cell containing "Classification: Confidential" or
       "Classification:" with the activity_name.
    2. Replace any standalone date cell (matching YYYY-MM-DD or DD-MM-YYYY)
       that is in a cover-page table (has a 'Date' label nearby) with today_str.
    """
    import re as _re
    date_pat = _re.compile(r'^\d{4}-\d{2}-\d{2}$|^\d{2}-\d{2}-\d{4}$')

    for table in doc.tables:
        flat_headers = [c.text.strip().lower() for c in table.rows[0].cells]
        # ── Revision table ──────────────────────────────────────
        if "version no." in flat_headers or "version no" in flat_headers:
            if len(table.rows) >= 2:
                row = table.rows[1]
                # Revision Date (col index 1)
                _set_cell_text(row.cells[1], today_str)
                # Description of Change (col index 3 or last)
                desc_idx = min(3, len(row.cells) - 1)
                _set_cell_text(row.cells[desc_idx], activity_name)
            continue

        # ── Cover page table — scan every cell ──────────────────
        found_date_label = any("date" in h for h in flat_headers)
        for row in table.rows:
            for cell in row.cells:
                txt = cell.text.strip()
                # Replace Classification: Confidential → activity_name
                if txt.lower().startswith("classification"):
                    _set_cell_text(cell, activity_name)
                # Replace cover-page Date value → today_str
                elif date_pat.match(txt) and found_date_label:
                    _set_cell_text(cell, today_str)


def _update_revision_table(doc: Document, activity_name: str, today_str: str):
    """Kept for backwards compatibility — now calls _update_cover_page."""
    _update_cover_page(doc, activity_name, today_str)
...
def build_mop(
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        if sec_key == "sop":
            # SOP: inject media at placeholder positions
            ...
            # ── UAT Section: format as numbered items with (a)(b)(c)(d) sub-points ──
            # Group consecutive (a)(b)(c)(d) lines under numbered headings.
            # A new item group starts when we see a line beginning with:
            #   - a digit/number (e.g. "1.", "2.")
            #   - "(a)" or "a)" — first sub-point of a new group (implies item break)
            # Each group is rendered as:
            #   [N]. [title derived from (a) Condition line]
            #        (a) Condition: ...
            ...
            def _make_uat_heading(doc, num, title_text):
                """Bold numbered item heading — e.g. '1.  Site Connectivity Check'"""
                p = OxmlElement("w:p")
                pPr = OxmlElement("w:pPr")
                ...
                t = OxmlElement("w:t")
                t.set("{http://www.w3.org/XML/1998/namespace}space", "preserve")
                t.text = f"{num}.  {title_text}"
                r.append(t); p.append(r)
                return p

            def _make_uat_subline(doc, line_text):
                """Indented sub-line for (a)(b)(c)(d) points."""
                p = OxmlElement("w:p")
                pPr = OxmlElement("w:pPr")
                ind = OxmlElement("w:ind"); ind.set(qn("w:left"), "360")
                sp = OxmlElement("w:spacing"); sp.set(qn("w:before"), "0"); sp.set(qn("w:after"), "20")
                pPr.append(ind); pPr.append(sp)
                p.append(pPr)
                r = OxmlElement("w:r")
                rpr = OxmlElement("w:rPr")
                fn = OxmlElement("w:rFonts"); fn.set(qn("w:ascii"), "Calibri"); fn.set(qn("w:hAnsi"), "Calibri")
            ...
            # ── Parse paragraphs into groups ──────────────────────────────────
            # A "group" is a list of lines that belong to one numbered UAT item.
            # We detect group boundaries by:
            #   1. A line that starts with a digit followed by "." or ")"
            #   2. A line starting with "(a)" after a non-(a) line (new block)
            groups = []   # list of lists of text strings
            current_group = []

            _numbered_re  = _re.compile(r'^\d+[\.\)]\s+\S')
            _sub_a_re     = _re.compile(r'^\(?a[\.\)]\s*', _re.IGNORECASE)
            _sub_any_re   = _re.compile(r'^\(?[a-dA-D][\.\)]\s*')
            _seen_a       = False
            ...
            for p_elem in content_elems:
                ...
                is_numbered = bool(_numbered_re.match(txt))
                is_sub_a    = bool(_sub_a_re.match(txt))
                is_sub_any  = bool(_sub_any_re.match(txt))

                if is_numbered:
                    # Explicit numbered heading — start new group
                    if current_group:
                        groups.append(current_group)
                    # Strip the leading number for the group lines; we re-number
                    stripped = _re.sub(r'^\d+[\.\)]\s+', '', txt)
                    current_group = [stripped]
                    _seen_a = False
                elif is_sub_a and _seen_a:
                    # Starting a new (a) after we already saw one — new group
                    if current_group:
                        groups.append(current_group)
                    current_group = [txt]
                    _seen_a = True
            ...
            # ── Render groups into document ───────────────────────────────────
            item_num = 0
            for group in groups:
                if not group:
                    continue

                # Determine if the first line of the group is a title or (a) line
                first = group[0]
                is_first_sub = bool(_sub_any_re.match(first))

                if is_first_sub:
                    # No explicit title — derive from (a) Condition line
                    item_num += 1
                    # Extract condition description after "Condition:"
                    cond_match = _re.search(r'[Cc]ondition\s*:\s*(.+)', first)
                    title = cond_match.group(1).strip() if cond_match else first
                ...
                heading_xml = _make_uat_heading(doc, item_num, title)
                _insert_after(cursor, heading_xml)
                cursor = heading_xml
                ...
                for sub in sub_lines:
                    if not sub.strip():
                        continue
                    sub_xml = _make_uat_subline(doc, sub)
                    _insert_after(cursor, sub_xml)
                    cursor = sub_xml

                # Blank spacer between items
                spacer = OxmlElement("w:p")
                _insert_after(cursor, spacer)
                cursor = spacer
