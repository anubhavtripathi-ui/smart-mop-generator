...
# Fixed numId used for ALL list paragraphs in the output MOP.
# abstractNumId=9 in the template defines:
#   ilvl=0 → decimal  (1, 2, 3 …)   — direct points under a heading
#   ilvl=1 → lowerLetter (a, b, c …) — sub-points under a sub-heading
# This constant is injected into every ListParagraph's numPr by _clone_para.
# If the template changes, update this value to match the correct numId.
_MOP_NUM_ID = 26   # numId=26 → abstractNumId=9 (decimal / lowerLetter)
...
def extract_mop_sections(mop_bytes: bytes) -> dict:
    ...
    for child in body_elem:
        ...
        if tag == "p":
            ...
            # Flatten table rows → plain paragraph elements
            for row in child.findall(f".//{{{_W}}}tr"):
                cells = row.findall(f".//{{{_W}}}tc")
                cell_texts = []
                for cell in cells:
                    ct = "".join(
                        t.text or "" for t in cell.findall(f".//{{{_W}}}t")
                    ).strip()
                    if ct:
...
def _update_revision_table(doc: Document, activity_name: str, today_str: str):
    ...
    """
    Deep-clone a paragraph and enforce MOP house style:
      • Font  : Calibri 10pt on all body runs
      • Color : 000000 (black) — strips red/blue/colored source text
      • Bold  : kept only for standalone sub-heading labels (full-para bold runs)
      • Italic / underline / highlight: stripped
      • numPr : ListParagraph paragraphs → numId remapped to _MOP_NUM_ID
                  ilvl=0  →  1, 2, 3  (decimal)
                  ilvl=1  →  a, b, c  (lowerLetter)
      • Headings: skipped — handled by _apply_heading_color separately
    """
    cloned = deepcopy(src_elem)
    _W = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"

    pStyle_elem = cloned.find(".//" + qn("w:pStyle"))
    style_val   = pStyle_elem.get(qn("w:val"), "") if pStyle_elem is not None else ""
    is_heading  = "Heading" in style_val
    is_list     = "List" in style_val or cloned.find(".//" + qn("w:numPr")) is not None

    full_para_text = "".join(
        t.text or ""
        for t in cloned.findall(f".//{{{_W}}}t")
    ).strip()

    # ── numPr normalization: remap to _MOP_NUM_ID ────────────────────────────
    if is_list and not is_heading:
        numPr = cloned.find(".//" + qn("w:numPr"))
        if numPr is not None:
            numId_el = numPr.find(qn("w:numId"))
            if numId_el is not None:
                numId_el.set(qn("w:val"), str(_MOP_NUM_ID))
            # ilvl: keep existing (0 → decimal 1,2,3 / 1 → lowerLetter a,b,c)
    ...
    for run in cloned.findall(f".//{{{_W}}}r"):
        ...
        if not is_heading:
            ...
            fn_new.set(qn("w:ascii"), "Calibri")
            fn_new.set(qn("w:hAnsi"), "Calibri")
            rpr.insert(0, fn_new)
            ...
            rpr[-1].set(qn("w:val"), "20")
            rpr.append(OxmlElement("w:szCs"))
            rpr[-1].set(qn("w:val"), "20")

            # Color → always black
            for col in rpr.findall(f"{{{_W}}}color"):
                rpr.remove(col)
            col_new = OxmlElement("w:color")
            col_new.set(qn("w:val"), "000000")
            rpr.append(col_new)

            # Bold: keep only if this run IS the entire paragraph text
            ...
            if b_el is not None:
                ...
                if not is_label:
                    rpr.remove(b_el)
            for bcs in rpr.findall(f"{{{_W}}}bCs"):
                rpr.remove(bcs)

            # Strip italic / underline / highlight
            for tag in (f"{{{_W}}}i", f"{{{_W}}}iCs",
                        f"{{{_W}}}u", f"{{{_W}}}highlight"):
                for el in rpr.findall(tag):
                    rpr.remove(el)
...
def _clone_para(src_elem: etree._Element) -> etree._Element:
    """
    Deep-clone a paragraph and enforce MOP house style:
      • Font  : Calibri 10pt on all body runs
      • Color : 000000 (black) — strips red/blue/colored source text
    ...
    for run in cloned.findall(f".//{{{_W}}}r"):
        ...
        if not is_heading:
            ...
            sz_new = OxmlElement("w:sz"); sz_new.set(qn("w:val"), "20")
            szcs_new = OxmlElement("w:szCs"); szcs_new.set(qn("w:val"), "20")
            rpr.append(sz_new); rpr.append(szcs_new)

            for col in rpr.findall(f"{{{_W}}}color"): rpr.remove(col)
            col_new = OxmlElement("w:color"); col_new.set(qn("w:val"), "000000")
            rpr.append(col_new)

            b_el = rpr.find(f"{{{_W}}}b")
            if b_el is not None:
                is_label = (run_text and run_text == full_para_text and len(run_text) < 80)
                if not is_label:
                    rpr.remove(b_el)
            for bcs in rpr.findall(f"{{{_W}}}bCs"): rpr.remove(bcs)

            for tag in (f"{{{_W}}}i", f"{{{_W}}}iCs", f"{{{_W}}}u", f"{{{_W}}}highlight"):
                for el in rpr.findall(tag): rpr.remove(el)
...
def build_mop(
    ...
    # Discover (or inject) the decimal/lowerLetter numbering definition
    # and set the module-level _MOP_NUM_ID so _clone_para uses it
    global _MOP_NUM_ID
    _MOP_NUM_ID = _ensure_mop_numbering(doc)
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        for child in list(body):
            ...
            if ctag == "p":
                ...
                to_remove.append(child)   # remove template placeholder tables
        for e in to_remove:
            body.remove(e)
        ...
        # ── Helper: insert p or tbl element after cursor ──────────────────────
        ...
        if sec_key == "sop":
            # ── SOP: MOP content first (if any), then Solution Doc content ────
            seen_texts = set()

            # 1. Activity MOP SOP content
            if mop_sections:
            ...
            # 2. Solution doc SOP content (deduplicated)
            ...
            # 3. Media: images/attachments at placeholders
            for p_elem in content_elems:
                if p_elem.tag.split("}")[-1] == "tbl":
                    continue
                ...
                if is_ph and media_idx < len(media_queue):
                    ...
                    if media_item.kind == "image":
                        try:
                            ...
                            desc = f"Image #{media_item.position_index+1}"
            ...
            # 2. Solution doc content (deduplicated)
            for p_elem in content_elems:
                tag = p_elem.tag.split("}")[-1]
                if tag == "tbl":
