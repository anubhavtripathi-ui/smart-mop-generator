...
def _update_header_date(doc: Document, today_str: str):
    ...
    # 2. Cover page table Date cell — find any table that has a "Date" label
    #    and whose cell contains a date-looking value, then update it
    for table in doc.tables:
        flat = [c.text.strip().lower() for c in table.rows[0].cells]
        if "version no." in flat or "version no" in flat:
            continue  # skip revision table, handled separately
        has_date_col = any("date" in h for h in flat)
        if not has_date_col:
            # also scan all cells for a "Date" label in any row
            found_date_label = False
            ...
            if not found_date_label:
                continue
        # Replace date-looking cell values with today_str
        for row in table.rows:
...
def build_mop(
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        if sec_key == "sop":
            ...
            # Collect all text lines, skip empty
            raw_lines = [_get_txt(p) for p in content_elems]
            raw_lines = [l for l in raw_lines if l]

            # Group into items: new group when we see explicit number OR new (a) after prior (a)
            groups = []
            cur    = []
            seen_a = False
            for line in raw_lines:
                ...
                is_sub_a = sub_m and sub_m.group(1).lower() == 'a'

                if is_num:
                    if cur: groups.append(cur)
                    cur = [_num_re.sub('', line)]
                    seen_a = False
            ...
            for grp in groups:
                ...
                if is_sub:
                    # derive title from condition line
                    cond = _re2.search(r'[Cc]ondition\s*[:\-]\s*(.+)', first)
                    title = cond.group(1).strip()[:80] if cond else first[:80]
                    sub_lines = grp
                else:
                    title     = first[:80]
                    sub_lines = grp[1:]
