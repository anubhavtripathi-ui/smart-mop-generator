...
# ─────────────────────────────────────────────────────────────────
# WORD NATIVE FLOWCHART RENDERER
# Detects ASCII-text diagrams in solution-doc paragraphs and replaces
# them with editable Word AutoShapes (rounded rect / diamond / rect)
# connected by arrow connectors — top-to-bottom layout.
# ─────────────────────────────────────────────────────────────────

# Regex patterns that signal an ASCII diagram line
_ASCII_FLOW_RE = re.compile(
    r'[\+\|\-\*]{3,}'           # box borders: +---+, |, ---
    r'|[-─]{4,}'                # long dashes
    r'|[┌┐└┘├┤┬┴┼│─]{2,}'      # box-drawing unicode chars
    r'|^\s*[▼▽↓↑→←▶◀►◄]{1}'    # arrow symbols at line start/end
    r'|\[.*?\].*?[-─→▼▶]'       # [Box] → style
    r'|[-─→▼▶].*?\[.*?\]'       # → [Box] style
    r'|\(\s*.+?\s*\).*?[-→▼]'   # (diamond) → style
    r'|[-→▼].*?\(\s*.+?\s*\)'   # → (diamond) style
...
def _is_ascii_flow_line(text: str) -> bool:
    """Return True if the text line looks like part of an ASCII flowchart."""
    if not text:
        return False
    stripped = text.strip()
    # Must have at least one flow indicator
    if _ASCII_FLOW_RE.search(stripped):
        return True
    # Pure box-border lines (e.g. "+-------+")
    if re.match(r'^[\s\+\-\|=*─│╔╗╚╝╠╣╦╩╬]+$', stripped) and len(stripped) >= 3:
        return True
    return False


def _detect_ascii_flowchart_runs(elements: list) -> list:
    """
    Scan a list of XML elements and return a list of (start_idx, end_idx) tuples
    identifying contiguous runs of ASCII-diagram paragraphs (≥ 3 lines).
    Tables are never included in a flow run.
    """
    _W = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    runs   = []
    in_run = False
    ...
    for i, elem in enumerate(elements):
        ...
        if tag == "tbl":
            if in_run and (i - start) >= 3:
                runs.append((start, i))
            in_run = False
            continue

        text = "".join(t.text or "" for t in elem.findall(f".//{{{_W}}}t")).strip()
        is_flow = _is_ascii_flow_line(text)
    ...
    if in_run and (len(elements) - start) >= 3:
        runs.append((start, len(elements)))

    return runs


def _extract_flow_steps(elements: list, start: int, end: int) -> list:
    """
    Parse ASCII-diagram paragraph elements [start:end] into a list of dicts:
      { 'text': str, 'shape': 'start_end'|'decision'|'process' }
    Shape classification:
      • 'start_end'  → Rounded Rectangle  (first, last, or text like Start/End/Begin/Finish)
      • 'decision'   → Diamond            (contains ?, YES, NO, or is wrapped in () or <>)
      • 'process'    → Rectangle          (everything else)
    Only lines with non-trivial text that aren't pure border/arrow lines are kept.
    """
    _W = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    steps = []
    elems = elements[start:end]

    for elem in elems:
        raw = "".join(t.text or "" for t in elem.findall(f".//{{{_W}}}t"))
        # Strip ASCII border characters, keep inner text
        text = re.sub(r'[+|\-=*─│┌┐└┘├┤┬┴┼╔╗╚╝╠╣╦╩╬▼▽↓↑→←▶◀►◄]', ' ', raw)
        text = re.sub(r'\s{2,}', ' ', text).strip()
        # Remove bracket/paren wrappers used in ASCII (e.g. [Start], (Decision?))
        text = re.sub(r'^[\[\(<]+\s*|\s*[\]\)>]+$', '', text).strip()
        if not text or len(text) < 2:
            continue
        # Skip pure YES/NO connector labels (they're branch labels, not steps)
        if re.match(r'^(yes|no|y|n|true|false)$', text, re.IGNORECASE):
            continue

        # Classify shape
        raw_stripped = raw.strip()
        is_terminal = bool(re.search(
            r'\bstart\b|\bend\b|\bbegin\b|\bfinish\b|\bterminate\b|\bstop\b',
            text, re.IGNORECASE))
        is_decision = bool(
            re.search(r'\?|yes|no|\bif\b|\bcheck\b|\bverify\b|\bdecide\b|\bdecision\b',
                      text, re.IGNORECASE)
            or re.match(r'^[\(\<].*[\)\>]$', raw_stripped)
        )

        if not steps and is_terminal:
            shape = "start_end"
        elif is_terminal:
            shape = "start_end"
        elif is_decision:
        ...
        steps.append({"text": text, "shape": shape})

    # Force first and last real steps to be start_end (terminal) if not already
    if steps:
        if steps[0]["shape"] == "process":
            steps[0]["shape"] = "start_end"
        if steps[-1]["shape"] == "process":
            steps[-1]["shape"] = "start_end"

    return steps


def _build_word_flowchart_xml(steps: list) -> str:
    """
    Build a complete <w:p> element (as XML string) containing a Word Drawing
    that holds all flowchart shapes + arrow connectors as a group inside a
    <mc:AlternateContent> / <w:drawing> block.

    Layout: top-to-bottom, shapes stacked vertically with arrows between them.
    Units: EMU (English Metric Units). 1 inch = 914400 EMU.

    Shape types (Word/DrawingML preset geometry):
      • start_end → roundRect
      • decision  → diamond
      • process   → rect
    """
    ...
    BOX_W      = int(2.8  * 914400)   # 2.8 inches wide
    BOX_H      = int(0.55 * 914400)   # 0.55 inches tall
    DIA_W      = int(2.8  * 914400)   # diamond same width
    DIA_H      = int(0.7  * 914400)   # diamonds slightly taller
    GAP        = int(0.4  * 914400)   # gap between boxes
    ARR_H      = int(0.3  * 914400)   # arrow height (gap region)
    LEFT_OFF   = int(0.5  * 914400)   # left margin inside drawing canvas
    TOP_OFF    = int(0.2  * 914400)   # top margin

    # Pre-compute y positions
    y_positions = []
    y = TOP_OFF
    for step in steps:
        h = DIA_H if step["shape"] == "decision" else BOX_H
        y_positions.append(y)
        y += h + GAP

    total_h = y + TOP_OFF
    total_w  = BOX_W + 2 * LEFT_OFF
    ...
    # Colour scheme
    FILL_COLORS = {
        "start_end": "003366",   # Ericsson navy
        "decision":  "0082C8",   # Ericsson blue
    ...
    def _sp(sid, shape_type, x, y, w, h, text, fill_hex, text_hex, border_hex):
        """Build a single <wps:wsp> shape XML string."""
        geom_map = {
            "start_end": "roundRect",
            "decision":  "diamond",
            "process":   "rect",
        }
        geom = geom_map.get(shape_type, "rect")
        # For roundRect add adjLst for corner radius
        adj_xml = ""
        if geom == "roundRect":
            adj_xml = '<a:avLst><a:gd name="adj" fmla="val 16667"/></a:avLst>'
        else:
            adj_xml = "<a:avLst/>"

        # Escape text for XML
        safe_text = text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        font_size = "1600"  # 8pt in hundredths-of-a-point

        return f"""<wps:wsp xmlns:wps="{NS['wps']}" xmlns:a="{NS['a']}">
  ...
  <wps:spPr>
    ...
    <a:prstGeom prst="{geom}">{adj_xml}</a:prstGeom>
  ...
  <wps:bodyPr anchor="ctr" lIns="91440" rIns="91440" tIns="45720" bIns="45720">
    <a:normAutofit/>
  </wps:bodyPr>
</wps:wsp>"""

    def _arrow(x1, y1, x2, y2):
        """Build a connector arrow from (x1,y1) to (x2,y2)."""
        cx = abs(x2 - x1)
        cy = abs(y2 - y1)
        off_x = min(x1, x2)
        off_y = min(y1, y2)
        # If purely vertical, cx is 0 — Word needs at least 1 EMU
        if cx == 0:
            cx = 1
        return f"""<wps:wsp xmlns:wps="{NS['wps']}" xmlns:a="{NS['a']}">
  ...
  <wps:spPr>
    <a:xfrm>
      <a:off x="{_emu(off_x)}" y="{_emu(off_y)}"/>
  ...
  <wps:bodyPr/>
    ...
    # Build all shape XML strings
    shape_xmls = []
    for i, step in enumerate(steps):
        h       = DIA_H if step["shape"] == "decision" else BOX_H
        w       = DIA_W if step["shape"] == "decision" else BOX_W
        x_off   = LEFT_OFF + (BOX_W - w) // 2   # centre diamonds same as rects
        y_top   = y_positions[i]
        fill    = FILL_COLORS[step["shape"]]
        tc      = TEXT_COLORS[step["shape"]]
        shape_xmls.append(_sp(
            next_id(), step["shape"],
            x_off, y_top, w, h,
            step["text"], fill, tc, BORDER_COLOR
        ))

        # Arrow to next step
        if i < len(steps) - 1:
            next_h   = DIA_H if steps[i+1]["shape"] == "decision" else BOX_H
            arr_x1   = LEFT_OFF + BOX_W // 2          # centre x
            arr_y1   = y_top + h                       # bottom of current shape
            arr_y2   = y_positions[i + 1]              # top of next shape
            shape_xmls.append(_arrow(arr_x1, arr_y1, arr_x1, arr_y2))

    # Wrap everything in wpg:wgp (Word Processing Group) inside a w:drawing / wp:inline
    inner_spTree = "\n".join(
        f"""<wpg:wsp xmlns:wpg="{NS['wpg']}">{s}</wpg:wsp>""" if False else s
        for s in shape_xmls
    )

    # Build the wpg:wgp group container
    group_xml = f"""<wpg:wgp xmlns:wpg="{NS['wpg']}" xmlns:a="{NS['a']}">
  <wpg:cNvGrpSpPr/>
  <wpg:grpSpPr>
    <a:xfrm>
      <a:off x="0" y="0"/>
      <a:ext cx="{_emu(total_w)}" cy="{_emu(total_h)}"/>
      <a:chOff x="0" y="0"/>
      <a:chExt cx="{_emu(total_w)}" cy="{_emu(total_h)}"/>
    </a:xfrm>
  </wpg:grpSpPr>
  {chr(10).join(shape_xmls)}
</wpg:wgp>"""

    # Wrap in w:drawing / wp:inline
    drawing_xml = f"""<w:drawing xmlns:w="{NS['w']}" xmlns:wp="{NS['wp']}" xmlns:wpg="{NS['wpg']}">
  <wp:inline distT="0" distB="0" distL="0" distR="0">
    <wp:extent cx="{_emu(total_w)}" cy="{_emu(total_h)}"/>
    <wp:effectExtent l="0" t="0" r="0" b="0"/>
    <wp:docPr id="9001" name="FlowChart"/>
...
def _replace_ascii_flowcharts_in_elements(doc: Document, elements: list) -> list:
    """
    Given a list of XML elements (paragraphs + tables from one section),
    detect ASCII-diagram runs and replace them with Word native flowchart
    paragraph elements. Returns the transformed list.

    Replaces detected ASCII blocks in-place, preserving all other elements.
    """
    if not elements:
        return elements

    runs = _detect_ascii_flowchart_runs(elements)
    if not runs:
        return elements

    result  = list(elements)  # shallow copy so we can splice
    # Process in reverse order so indices remain valid after splicing
    for start, end in reversed(runs):
        steps = _extract_flow_steps(elements, start, end)
        if len(steps) < 2:
            continue  # not enough steps — leave as-is

        chart_xml = _build_word_flowchart_xml(steps)
        if not chart_xml:
            continue

        try:
            chart_elem = etree.fromstring(chart_xml.encode("utf-8"))
        except Exception:
            continue

        # Replace the ASCII block [start:end] with a single chart paragraph
        result[start:end] = [chart_elem]
...
def build_mop(
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        # ── Replace ASCII flowcharts with Word native AutoShapes ──────────────
        # Run detection on solution-doc content (ASCII diagrams come from Copilot
        # output, not from Activity MOP uploads which contain real images).
        content_elems = _replace_ascii_flowcharts_in_elements(doc, list(content_elems))
