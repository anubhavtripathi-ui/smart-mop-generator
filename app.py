...
# PASSWORD GATE
# ─────────────────────────────────────────────────────────────────
_CORRECT_PASSWORD = "Ericsson@2024"

if not st.session_state["authenticated"]:
    st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Lato:wght@300;400;700;900&display=swap');
    html, body, [class*="css"] { font-family: 'Lato', sans-serif; background-color: #001f4d; }
    ...
    st.markdown("""
    <div style="min-height:100vh; background:linear-gradient(135deg,#001f4d 0%,#003366 60%,#004080 100%);
                display:flex; align-items:center; justify-content:center; padding:40px;">
      <div style="background:rgba(255,255,255,0.06); border:1px solid rgba(0,130,200,0.25);
                  border-radius:20px; padding:52px 56px; max-width:460px; width:100%; text-align:center;">
        <div style="font-family:'Lato',sans-serif; font-weight:900; font-size:2rem;
                    letter-spacing:6px; color:#0082C8; margin-bottom:4px;">ERICSSON</div>
        <div style="font-size:0.6rem; letter-spacing:2px; color:rgba(255,255,255,0.3);
                    text-transform:uppercase; margin-bottom:28px;">Technology For Good</div>
        <div style="font-size:1.1rem; font-weight:700; color:#ffffff; margin-bottom:6px;">
            Smart MOP Generator</div>
        <div style="font-size:0.78rem; color:rgba(255,255,255,0.45); margin-bottom:36px; line-height:1.6;">
    ...
    with col:
        st.markdown(
            ...
            'color:rgba(255,255,255,0.4);margin-bottom:6px;">Access Password</p>',
            unsafe_allow_html=True,
        )
...
def extract_sections(doc: Document) -> dict:
    """Extract section content as XML elements preserving all run formatting."""
...
def _apply_heading_color(p_elem):
    """Apply heading colour to both pPr>rPr (paragraph-level) and each w:r>rPr (run-level)."""
    def _fix_color(rpr):
    ...
    # Fix paragraph-level rPr (inside pPr) — this overrides run-level in some renderers
...
def _update_revision_table(doc: Document, activity_name: str, today_str: str):
    for table in doc.tables:
        if "Version No." not in [c.text.strip() for c in table.rows[0].cells]:
            continue
        if len(table.rows) >= 2:
            row = table.rows[1]
...
def build_mop(template_bytes: bytes, activity_name: str,
    ...
    if title_elem is not None:
        ...
        r = sub.add_run(activity_name)
        r.font.name = "Calibri"
        r.font.size = Pt(14)
        r.font.italic = True
        r.font.underline = True
        sub_e = sub._element
        sub_e.getparent().remove(sub_e)
        title_elem.addnext(sub_e)

    _update_revision_table(doc, activity_name, today_str)

    # Map Heading1 elements to section keys
    heading_map = {}
    for child in list(body):
        ...
        if key and key != "connectivity_diagram":
            heading_map[id(child)] = (child, key)

    # Apply colour to ALL Heading1 elements (including Revision History)
    for child in list(body):
        if child.tag.split("}")[-1] != "p":
            continue
        se = child.find(".//" + qn("w:pStyle"))
        if se is not None and se.get(qn("w:val"), "") == "Heading1":
            _apply_heading_color(child)

    for _, (h_elem, sec_key) in heading_map.items():
        # Remove template example content under this heading
        ...
        # Blue colour on heading
        _apply_heading_color(h_elem)

        # Page break before Objective (first content section = new page)
        ...
        # Inject cloned paragraphs (reversed so order is preserved after addnext)
        content_elems = sections.get(sec_key, [])
        if not content_elems:
            h_elem.addnext(OxmlElement("w:p"))
        else:
            for p_elem in reversed(content_elems):
                h_elem.addnext(_clone_para(p_elem))

    # Connectivity diagrams
    for img_bytes, ext in sections.get("connectivity_diagram", []):
        p = doc.add_paragraph()
