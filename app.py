...
def build_mop(
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        if sec_key == "sop":
            ...
            # Append extra reference file notices at SOP end
            if extra_ref_names:
                sep = _make_xml_para(doc, "", size_pt=10)
                _insert_after(cursor, sep)
                cursor = sep
                hdr = _make_xml_para(
                    doc,
                    "── ADDITIONAL REFERENCE FILES UPLOADED ──",
                    bold=True, color_rgb="003366", size_pt=10
                )
                _insert_after(cursor, hdr)
                cursor = hdr
                for ref_name in extra_ref_names:
                    ref_xml = _make_xml_para(
                        doc,
                        f"\U0001f4cb  Reference: {ref_name} — please review and incorporate manually if applicable.",
                        italic=True, color_rgb="555555", size_pt=9
                    )
                    _insert_after(cursor, ref_xml)
                    cursor = ref_xml
        else:
            for p_elem in content_elems:
                cloned = _clone_para(p_elem)
...
with col_left:
    ...
    # ── Step 4: Additional References (OPTIONAL) ─────────────────
    st.markdown('<div class="eri-card"><div class="eri-card-title"><span class="step-badge">STEP 04</span> Upload Additional References &nbsp;<span class="optional-badge">OPTIONAL</span></div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="font-size:0.73rem;color:#5a7a9a;margin-bottom:0.5rem;">
      Upload any additional supporting files: logs, screenshots, tables, flowcharts,
      network diagrams, or any reference documents. These will be listed as reference
      attachments at the end of the SOP section in the output MOP.
    </div>
    """, unsafe_allow_html=True)

    extra_files = st.file_uploader(
        "Additional files (logs, images, tables, etc.) — optional",
        type=["txt", "log", "png", "jpg", "jpeg", "xlsx", "xls", "csv", "pdf", "docx"],
    ...
    if extra_files:
        for ef in extra_files:
            ...
            st.markdown(
                f'<div class="pill-info">📎 &nbsp;<strong>{ef.name}</strong>'
                f' &nbsp;·&nbsp; {size_kb:.1f} KB</div>',
                unsafe_allow_html=True,
            )
    else:
        st.markdown('<div class="pill-info">ℹ No additional files uploaded.</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # ── Step 5: Generate ─────────────────────────────────────────
    st.markdown('<div class="eri-card"><div class="eri-card-title"><span class="step-badge">STEP 05</span> Generate Smart MOP</div>', unsafe_allow_html=True)
...
with col_right:
    ...
    if gen_btn and can_go:
        ...
        steps = [
            ...
            "Processing additional reference files" if has_extra else "No additional files",
        ...
        try:
            ...
            c_injected      = []
            c_failed        = []
            extra_ref_names = [ef.name for ef in extra_files] if extra_files else []
            tmpl_b          = None
            ...
            st.session_state["extra_refs"]        = extra_ref_names
    ...
    if st.session_state.get("output_bytes"):
        ...
        extra_refs        = st.session_state.get("extra_refs", [])
        ...
        if extra_refs:
            st.markdown(
                f'<div class="pill-info" style="margin-top:0.4rem;">'
                f'📎 &nbsp;{len(extra_refs)} additional reference file(s) listed in SOP section</div>',
                unsafe_allow_html=True
            )
