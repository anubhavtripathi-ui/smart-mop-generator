...
def extract_sections(doc: Document) -> dict:
    ...
    for child in body_elem:
        ...
        # Only Heading1 defines section boundaries. A plain paragraph containing
...
def _clone_para(src_elem, num_map: dict = None, full_para_text: str = None):
    ...
    # These come from source docs and render as dark horizontal lines in output.
...
def build_mop(
    ...
    for child in list(body):
        ...
        if se is not None and se.get(qn("w:val")) == "Title":
            # ── Add text underline to Title runs (text-width only, not full page) ──
            for r_el in child.findall(".//" + qn("w:r")):
                rpr_el = r_el.find(qn("w:rPr"))
                if rpr_el is None:
                    rpr_el = OxmlElement("w:rPr"); r_el.insert(0, rpr_el)
