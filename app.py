...
def _normalise_run(rpr, is_heading: bool = False):
    """Apply house-style to a single rPr element in-place."""
    _W = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    if is_heading:
        return
    # Font → Calibri
    ...
    szc = OxmlElement("w:szCs"); szc.set(qn("w:val"), "20"); rpr.append(szc)
    # Color → black
    for c in rpr.findall(f"{{{_W}}}color"):
        rpr.remove(c)
...
def _clone_para(src_elem, sec_num_id: str = None, full_para_text: str = None):
    """
    Deep-clone a paragraph applying house style.
    sec_num_id: if provided, remaps numPr to this section-specific numId
                so every section's list always starts at 1.
    full_para_text: pre-computed full text (for bold-label detection).
    """
    ...
    # Remap numId to section-specific fresh counter
    if sec_num_id and not is_heading:
        numPr = cloned.find(".//" + qn("w:numPr"))
        if numPr is not None:
            nid_el = numPr.find(qn("w:numId"))
            if nid_el is not None:
                nid_el.set(qn("w:val"), sec_num_id)
    ...
    for run in cloned.findall(f".//{{{_W}}}r"):
        ...
        if rpr is None:
            ...
            run.insert(0, rpr)
        run_text = "".join(
            t.text or "" for t in run.findall(f"{{{_W}}}t")
        ).strip()
        if not is_heading:
            _normalise_run(rpr)
            # Bold: keep ONLY for standalone sub-heading labels
...
def _clone_table(src_tbl):
    """
    Deep-clone a table and normalise all cell run formatting to house style.
    Preserves table structure, borders, cell widths exactly.
    """
    ...
    for run in cloned.findall(f".//{{{_W}}}r"):
        ...
        if rpr is None:
            ...
            run.insert(0, rpr)
        _normalise_run(rpr)
        # Strip bold inside table cells (use normal weight for data)
        for b in rpr.findall(f"{{{_W}}}b"):
            rpr.remove(b)
        for bcs in rpr.findall(f"{{{_W}}}bCs"):
            rpr.remove(bcs)
    return cloned
...
def build_mop(
    ...
    # One-time: find the decimal/lowerLetter abstractNumId
    _decimal_abstract_id = _get_decimal_abstract_id(doc)
    ...
    for child in list(body):
        ...
        if not text:                          # ← blank Heading1 = template separator, skip
            body.remove(child)
            continue
    ...
    for h_elem, sec_key in ordered_sections:
        ...
        # Fresh numId for THIS section so numbering always starts at 1
        sec_num_id = _new_section_num_id(doc, _decimal_abstract_id)

        # ── Universal element inserter ────────────────────────────────────────
        def _insert_elem(elem, cur, _snid=sec_num_id):
            ctag = elem.tag.split("}")[-1]
            if ctag == "tbl":
                tbl_clone = _clone_table(elem)
                ...
                cl = _clone_para(elem, sec_num_id=_snid, full_para_text=full_txt)
        ...
        if sec_key == "sop":
            ...
            # 1. Activity MOP SOP content first
            if mop_sections:
