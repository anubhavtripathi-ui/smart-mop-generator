...
HEADING_MAP = {
    "objective": ["objective","objectives","goal","goals","purpose","aim","overview","summary","introduction"],
    "activity":  ["activity description","activity","description","work description","task description",
                  "scope of work","activity steps","activity window","work summary","change description"],
    ...
    "prereq":    ["pre-requisites","prerequisites","pre requisites","preconditions","prerequisite",
                  "preparation","pre-requisites check list","prerequisites check list","pre-checks",
                  "pre checks","prechecks","pre requisite check"],
    "inventory": ["inventory details","inventory","node details","equipment details","node inventory",
                  "asset details","input sheet","node list"],
    "connect":   ["node connectivity process","connectivity","node connectivity","connection process",
                  "access method","scripts used","login","cli access"],
    "iam":       ["identity and access management","iam","access management","credentials",
                  "login details","authentication"],
    "trigger":   ["activity triggering method","trigger","triggering method","activity trigger",
                  "how to trigger"],
    "sop":       ["standard operating procedure","sop","procedure","operating procedure",
                  "detailed procedure","execution","activity execution","execution steps"],
    "accept":    ["acceptance criteria","acceptance test criteria","acceptance test","acceptance",
                  ...
                  "sign off criteria","sign-off criteria","test acceptance","post checks",
                  "post-checks","post requisites","post-requisites","postchecks"],
    "assume":    ["assumptions","assumption","assumed conditions","dependencies","notes","caveats",
                  "risk & impact","risk and impact","rollback","fallback","node deletion"],
}
...
def normalize(text):
    # Remove leading numbers like "1.", "10.", "1.1" etc from headings
...
def match_heading(heading_text):
    ...
    for key, synonyms in HEADING_MAP.items():
        for syn in synonyms:
            if norm == syn:
                return key
    # 2. Starts-with match (longest synonym wins)
    for key, synonyms in HEADING_MAP.items():
        for syn in synonyms:
    ...
    if best_key:
        return best_key
    # 3. Substring match (only for synonyms >= 8 chars)
    for key, synonyms in HEADING_MAP.items():
        for syn in synonyms:
            if len(syn) >= 8 and syn in norm and len(syn) > best_len:
                best_key, best_len = key, len(syn)
    return best_key
...
def _looks_like_heading(text):
    """Detect bold/caps lines that act as headings even if not styled as Heading."""
    t = text.strip()
...
def extract_content_from_docx(input_path):
    ...
    for para in doc.paragraphs:
        ...
        matched_key = None
        # ONLY match headings from styled Heading paragraphs or ALL CAPS lines
        # Never match Normal/List Paragraph content lines as headings
        if _is_heading_para(para):
            matched_key = match_heading(text)
        elif _looks_like_heading(text) and current_key is None:
            # ALL CAPS only considered heading if no section started yet
            matched_key = match_heading(text)
...
def extract_content_from_txt(input_path):
    ...
    for line in lines:
        ...
        if not text:
            continue
        # Only treat as heading if short enough to be a heading (<= 80 chars)
        candidate = text.rstrip(':')
        is_short = len(candidate) <= 80
        matched_key = match_heading(candidate) if is_short else None
        if matched_key:
...
def _create_full_input_copy(input_doc):
    """Create a new docx = complete copy of input (all content, OLE, images)."""
    new_doc = Document()
...
def copy_sop_content(output_doc, input_doc, input_path, activity_name):
    """SOP section: just add reference note. Full copy delivered as separate file."""
    token = '{{sop}}'
    placeholder_para = None
    for para in output_doc.paragraphs:
        if token in para.text:
