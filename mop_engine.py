...
def copy_sop_content(output_doc, input_doc, input_path, content_map):
    """
    SOP section:
    1. Add [Please attach original SOP here] note
    2. If input DOCX has SOP heading → copy only that section (no other headings repeated)
       If input DOCX has no SOP heading → copy content NOT already in other sections
    3. Create full copy of input doc → embed as attachment
    Returns: full_copy_doc (or None for txt)
    """
    token = '{{sop}}'
    placeholder_para = None
    for para in output_doc.paragraphs:
    ...
    parent.insert(insert_pos, _make_note_para(
        '[ Please attach the original SOP / input MOP document here ]', color='C00000'))
    insert_pos += 1
    ...
    if input_doc is None:
        try:
            ...
            if txt:
                ...
                parent.insert(insert_pos, p)
        except Exception:
            pass
        return None

    # DOCX — build body index structures
    input_body    = input_doc.element.body
    body_children = list(input_body)
    input_paras   = input_doc.paragraphs
    para_elements = [p._element for p in input_paras]

    # Find SOP heading range in body
    sop_start_idx = None
    sop_end_idx   = None
    for body_i, child in enumerate(body_children):
        tag = child.tag.split('}')[-1] if '}' in child.tag else child.tag
        if tag != 'p' or child not in para_elements: continue
        para_obj = input_paras[para_elements.index(child)]
        if 'heading' not in para_obj.style.name.lower(): continue
        matched = match_heading(para_obj.text.strip())
        if matched == 'sop':
            sop_start_idx = body_i + 1
        elif sop_start_idx is not None and sop_end_idx is None:
            sop_end_idx = body_i

    def elem_tag(c):
        return c.tag.split('}')[-1] if '}' in c.tag else c.tag

    def is_copyable(c):
        return elem_tag(c) in ('p', 'tbl', 'sdt')

    def elem_has_real_content(c):
        xml = etree.tostring(c).decode()
        txt = ''.join(re.findall(r'<w:t[^>]*>([^<]*)</w:t>', xml))
        has_rich = 'blip' in xml or 'OLEObject' in xml
        return has_rich or (txt.strip() and not is_junk(txt))

    if sop_start_idx is not None:
        # Copy ONLY SOP section content — guaranteed no other headings
        end = sop_end_idx if sop_end_idx else len(body_children)
        to_copy = [c for c in body_children[sop_start_idx:end]
                   if is_copyable(c) and elem_has_real_content(c)]
    else:
        # No SOP heading — find body indices covered by other extracted headings
        covered = set()
        current_key2 = None
        current_start2 = None
        for body_i, child in enumerate(body_children):
            ...
            matched = match_heading(para_obj.text.strip())
            if matched:
                if current_key2 and current_key2 in content_map:
                    for i in range(current_start2, body_i):
                        covered.add(i)
                current_key2  = matched
                current_start2 = body_i
        if current_key2 and current_key2 in content_map:
            for i in range(current_start2, len(body_children)):
                covered.add(i)

        # Copy everything not covered, skip known heading paras
        to_copy = []
        for body_i, c in enumerate(body_children):
            if body_i in covered or not is_copyable(c): continue
            # Skip heading paras that are already mapped to sections
            if c in para_elements:
                para_obj = input_paras[para_elements.index(c)]
                if 'heading' in para_obj.style.name.lower() and match_heading(para_obj.text.strip()) in content_map:
                    continue
            if elem_has_real_content(c):
                to_copy.append(c)

    # Insert elements — never fallback to full doc copy (avoids duplication)
    if to_copy:
        rId_map = _copy_rels_and_remap(to_copy, input_doc, output_doc)
        elems_copy = [deepcopy(e) for e in to_copy]
        _apply_rid_map(elems_copy, rId_map)
        for elem in elems_copy:
            parent.insert(insert_pos, elem)
            insert_pos += 1
    elif sop_start_idx is None:
        # No SOP heading and no content to copy — add a note
        parent.insert(insert_pos, _make_note_para(
            'No SOP content found in input. Please attach the original SOP document above.',
            color='808080'))

    # Create full input copy for embedding
    return _create_full_input_copy(input_doc)
...
def generate_mop(template_path, input_path, activity_name, vendor_name, output_path):
    ...
    # SOP — smart copy + embed full input copy
    full_copy_doc = copy_sop_content(output_doc, input_doc, input_path, content_map)

    replace_placeholders_in_doc(output_doc, simple_replacements)
    output_doc.save(output_path)
    _update_header_date(output_path)

    # Embed complete input copy as attachment
    if full_copy_doc is not None:
        attach_name = os.path.splitext(os.path.basename(input_path))[0] + '_original_input.docx'
        _embed_doc_in_zip(output_path, full_copy_doc, attach_name)
